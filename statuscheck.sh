...
WORKSPACE=/directory/where/script/
...
function test {
  response=$(curl -L --connect-timeout 5 --write-out %{http_code} --silent --output /dev/null $1)
#  disable above and enable below for HTTPS
#  response=$(curl -L --connect-timeout 5 --write-out %{http_code} --silent --output /dev/null https://$1)
